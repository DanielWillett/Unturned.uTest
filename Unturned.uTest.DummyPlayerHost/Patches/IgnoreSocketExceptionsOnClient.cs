using HarmonyLib;
using System;
using System.Diagnostics;
using System.Reflection;

#pragma warning disable IDE0130

namespace uTest.Dummies.Host.Patches;

/// <summary>
/// Patches "SDG.NetTransport.SystemSockets.ClientTransport_SystemSockets.Send" to prevent exceptions from escaping the send method, instead just disconnecting from the server.
/// </summary>
internal static class IgnoreSocketExceptionsOnClient
{
    private const string PatchName = "ClientTransport_SystemSockets.Send";
    private static bool _hasPatch;

    private static MethodInfo? _patchedMethod;

    internal static bool TryPatch(Harmony harmony, ILogger logger)
    {
        _hasPatch = false;
        Type? type = Type.GetType("SDG.NetTransport.SystemSockets.ClientTransport_SystemSockets, Assembly-CSharp", throwOnError: false, ignoreCase: false);
        if (type == null)
        {
            logger.LogError(string.Format(Properties.Resources.LogErrorPatchFailed, PatchName, "Unable to find type."));
            return false;
        }

        _patchedMethod = type.GetMethod(
            "Send",
            BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance,
            null,
            CallingConventions.Any,
            [ typeof(byte[]), typeof(long), typeof(ENetReliability) ],
            null
        );
        if (_patchedMethod == null)
        {
            logger.LogError(string.Format(Properties.Resources.LogErrorPatchFailed, PatchName, "Unable to find target method."));
            return false;
        }

        try
        {
            harmony.Patch(_patchedMethod, finalizer: new HarmonyMethod(new FinalizerDelegate(Finalizer).Method));
            _hasPatch = true;
        }
        catch (Exception ex)
        {
            logger.LogError(string.Format(Properties.Resources.LogErrorPatchFailed, PatchName, "Patch error."), ex);
            return false;
        }

        return true;
    }

    internal static bool TryUnpatch(Harmony harmony)
    {
        if (!_hasPatch)
            return false;

        try
        {
            harmony.Unpatch(_patchedMethod, new FinalizerDelegate(Finalizer).Method);
            return true;
        }
        catch
        {
            return false;
        }
        finally
        {
            _hasPatch = false;
            _patchedMethod = null;
        }
    }

    private delegate Exception? FinalizerDelegate(Exception? __exception);

    internal static Exception? Finalizer(Exception? __exception)
    {
        if (__exception == null)
            return null;


        UnturnedLog.warn("Exception sending message");
        UnturnedLog.warn(__exception);
        __exception = null;

        // this is awful but its fine ig, if currently disconnecting don't re-call it
        if (new StackTrace(2).GetFrame(0) is not { } frame || frame.GetMethod() != new Action(Provider.disconnect).Method)
        {
            UnturnedLog.info("Disconnecting next frame...");
            Task.Run(async () =>
            {
                await GameThread.Switch();
                Provider.disconnect();
            });
        }
        else
        {
            UnturnedLog.info("Not disconnecting because already disconnecting.");
        }

        return null;
    }
}

#pragma warning restore IDE0130