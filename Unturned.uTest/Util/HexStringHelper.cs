...
{
    // stole these from Uncreated
    //  https://github.com/UncreatedStaff/UncreatedWarfare/blob/uncreated-networking-cleanup/UncreatedWarfare/Util/HexStringHelper.cs
...
#else
            if (!float.TryParse(starterSection[1..].ToString(), NumberStyles.Number, formatProvider, out float r))
#endif
                goto fail;
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP2_1_OR_GREATER
            if (!float.TryParse(str[splitResults[1]], NumberStyles.Number, formatProvider, out float g))
#else
            if (!float.TryParse(str[splitResults[1]].ToString(), NumberStyles.Number, formatProvider, out float g))
#endif
                goto fail;
...
#else
            if (!float.TryParse(lastSection.ToString(), NumberStyles.Number, formatProvider, out float b))
#endif
                goto fail;
            if (commaCt == 3)
            {
...
#else
                if (!float.TryParse(lastSection.ToString(), NumberStyles.Number, formatProvider, out a))
#endif
                    goto fail;
            }
...
#else
            if (!int.TryParse(starterSection[1..].ToString(), NumberStyles.Number, formatProvider, out int r))
#endif
                goto fail;
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP2_1_OR_GREATER
            if (!int.TryParse(str[splitResults[1]], NumberStyles.Number, formatProvider, out int g))
#else
            if (!int.TryParse(str[splitResults[1]].ToString(), NumberStyles.Number, formatProvider, out int g))
#endif
                goto fail;
            ReadOnlySpan<char> lastSection = str[splitResults[2]];
            if (commaCt == 2 && lastSection[^1] == ')')
...
#else
            if (!int.TryParse(lastSection.ToString(), NumberStyles.Number, formatProvider, out int b))
#endif
                goto fail;
            if (commaCt == 3)
            {
...
#else
                if (!int.TryParse(lastSection.ToString(), NumberStyles.Number, formatProvider, out a))
#endif
                    goto fail;
            }
...
#else
            color = new Color32((byte)Mathf.Clamp(r, 0, 255), (byte)Mathf.Clamp(g, 0, 255), (byte)Mathf.Clamp(b, 0, 255), (byte)Mathf.Clamp(a, 0, 255));
#endif
            return true;
        fail:
            color = default;
    ...
    public static string FormatHexColor(Color32 color, int size = -1)
    {
        ...
        unsafe
        {
            char* strBuf = stackalloc char[size];
            FormatHexColor(color, new Span<char>(strBuf, size));
            return new string(strBuf, 0, size);
        }
#endif
    }
